            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-center mb-0.5">
                  ...
                  <ChevronRight className={`w-3 h-3 transition-transform ${mod.status === 'current' ? 'rotate-90' : 'group-hover:translate-x-1'}`} />
                </div>
                <h4 className="font-bold text-sm leading-tight group-hover:text-gold transition-colors flex items-center gap-2">
                  {mod.title}
                  {idx === 1 && (
                    <span className="text-[8px] px-1.5 py-0.5 bg-gold text-white rounded font-black tracking-widest leading-none">FREE</span>
                  )}
                </h4>
                <div className="mt-2 flex items-center gap-2 opacity-60">
                  <BookOpen className="w-3 h-3" />
                  <span className="text-[10px] font-bold">{mod.lessons} Lessons</span>
                </div>
              </div>
            </div>
          </div>
